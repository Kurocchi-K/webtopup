    ...
    const props = defineProps<{
        ...
        faqs: FaqDataItem[];
        currencyProducts: PPOBProductDataItem[];
        membershipProducts: PPOBProductDataItem[];
    }>();
    const page = usePage();
    ...
    const checkGameAccount = useDebounceFn(async () => {
        ...
        try {
            const response = await axios.post( '/check-game-account', {
                ...
                console.log("Respon API Frontend:", response.data);

            const username = response.data.data?.username || response.data.data?.data?.username;
            ...
            if (username) {
                ...
                checkError.value = response.data.message || 'Game ID tidak ditemukan';
                resolvedUsername.value = null;
            }
        } catch (error: any) {
    ...
    watch(
        ...
        () => {
            if (checkError.value || resolvedUsername.value) {
            checkError.value = null;
            resolvedUsername.value = null;
            }
            checkGameAccount();        
